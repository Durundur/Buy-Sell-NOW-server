...
router.get("/search/:mainCatParam?/:subCatParam?/:subSubCatParam?", async function (req, res, next) {
	...
	try {
		const ads = await AdModel.find(queryFilter)
...
router.get("/user", ensureAuthenticated, async function (req, res, next) {
	...
	try {
		const ads = await AdModel.find({ "advertiser._id": requestUserId })
			.skip(ADS_PER_PAGE * (page - 1))
			.limit(ADS_PER_PAGE);
		res.status(200).send(ads);
	} catch (error) {
...
router.get("/user/:id/:mainCatParam?/:subCatParam?/:subSubCatParam?", async function (req, res, next) {
	...
	try {
		const ads = await AdModel.find({ "advertiser._id": userId, ...categoryQuery })
			.skip(ADS_PER_PAGE * (page - 1))
			.limit(ADS_PER_PAGE)
			.sort([sortArr]);
		res.status(200).send(ads);
